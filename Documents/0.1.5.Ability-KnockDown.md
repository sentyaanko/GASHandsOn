...
* まだ扱わないことは以下の通り
	* インタラクション機能を実装していないので蘇生処理はまだ対応しない。
...
| タグ名                      | 用途                                                                                           |
|-----------------------------|------------------------------------------------------------------------------------------------|
| `Ability.BlocksInteraction` | インタラクション操作の抑制用タグ                                                               |
| `Ability.Revive`            | インタラクション操作による蘇生アビリティ用タグ（タグの用意だけで蘇生を行うアビリティは未実装） |
...
1. `GE_KnockedDown` の新規追加
	...
	* `GrantedTags` に `Ability.BlocksInteraction` を設定。
		* これにより、キャラクターをインタラクション操作を無効状態にする。
