...
#include "GHOAbilityTask_PlayMontageForMeshAndWaitForEvent.generated.h"


//1pv/3pv の切り替えをできるようにしていないため、この機能を使用するためのコードを無効化しています。
//クラス定義がないと generated.cpp/.h のビルドが動かないためダミーの型を定義しています。
UCLASS()
class GASHANDSON_API UGHOAbilityTask_PlayMontageForMeshAndWaitForEvent : public UAbilityTask
{
	GENERATED_BODY()
};
...
*/
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FGHOPlayMontageForMeshAndWaitForEventDelegate, FGameplayTag, EventTag, FGameplayEventData, EventData);
...
public:
	...
	UGHOAbilityTask_PlayMontageForMeshAndWaitForEvent(const FObjectInitializer& ObjectInitializer);
	...
	和訳
		このタスクの Blueprint ノードである PlayMontageForMeshAndWaitForEvent には、 AbilityTask.h で記載されているように、 
	...
	*/
	...
	FGHOPlayMontageForMeshAndWaitForEventDelegate OnCompleted;
	...
	*/
	UPROPERTY(BlueprintAssignable)
	FGHOPlayMontageForMeshAndWaitForEventDelegate OnBlendOut;
	...
	*/
	UPROPERTY(BlueprintAssignable)
	FGHOPlayMontageForMeshAndWaitForEventDelegate OnInterrupted;
	...
	*/
	UPROPERTY(BlueprintAssignable)
	FGHOPlayMontageForMeshAndWaitForEventDelegate OnCancelled;
	...
	*/
	UPROPERTY(BlueprintAssignable)
	FGHOPlayMontageForMeshAndWaitForEventDelegate EventReceived;
	...
	*/
	...
	static UGHOAbilityTask_PlayMontageForMeshAndWaitForEvent* PlayMontageForMeshAndWaitForEvent(
...
private:
	...
	和訳
		モンタージュが再生されているメッシュ。 AvatarActor が所有している必要があります。
	*/
	UPROPERTY()
	USkeletalMeshComponent* Mesh;
